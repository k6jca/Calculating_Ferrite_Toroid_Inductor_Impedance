...
Ytot = YZl + YCp;            % sum admittances
Zactual = squeeze(y2z(Ytot));% Final impedance (Z(inductor) paralleled with
...
sgt = sgtitle({'Calculated Impedance of ',comment1,['(shunt capacitance = ',Cp_text,' pF)'],''}, 'FontWeight','bold');  % Title on 3 lines

subplot(2,2,1);
plot(f*1e-6,abs(Zactual),'Color', [0.3010, 0.7450, 0.9330],'LineStyle','-','linewidth',2);
...
subplot(2,2,3);
plot(f*1e-6,(angle(Zactual)*180/pi),'Color', [0.3010, 0.7450, 0.9330],'LineStyle','-','linewidth',2);
...
subplot(2,2,2);
plot(f*1e-6,real(Zactual),'Color', [0.3010, 0.7450, 0.9330],'LineStyle','-','linewidth',2);
...
subplot(2,2,4);
plot(f*1e-6,imag(Zactual),'Color', [0.3010, 0.7450, 0.9330],'LineStyle','-','linewidth',2);
